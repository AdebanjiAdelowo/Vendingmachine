-- 1. Transactions with Products for a specific time period (StartDate and EndDate)
...
BEGIN
    SELECT -- Use /*+ ORDERED */ to tell Oracle to follow the join order written in the query, which will process the INNER JOIN first.
        ...
        T.TRN_MODALITAPAGAMENTO AS "Payment mode",
        ...
        T.TRN_ESITOTRANSAZIONE AS "Transaction Result",
        T.TRN_PREZZOAPPLICATO AS "Price Applied for Credit Trans",
        T.TRN_WALLET_BALANCE_DECLARED AS "Wallet Balance after Trans",
        T.TRN_DBVEGA AS "TRN DB VEGA",
        D.DBV_DESC AS "DB Vega Desc",
        T.TRN_PRO_VENDPAY AS "VenPay Product Code",
        T.TRN_DATAADDEBITO AS "Debit date",
        T.TRN_TIPO_SISTEMA_ESTERNO AS "External System Type",
